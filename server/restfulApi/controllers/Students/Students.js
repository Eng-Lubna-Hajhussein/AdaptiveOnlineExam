const {Students} = require("../../../database/models/database");
...
module.exports.signup_post = async (req, res) => {
  ...
  try {
    const userExists = await Students.findOne({ where: { email: email } });
    if(userExists){
      throw Error("user exists")
    }
    const salt = await bcrypt.genSalt();
    const hashedPassword = await bcrypt.hash(password, salt);
    const user = await Students.create({ email, password: hashedPassword, fullname });
...
module.exports.login_post = async (req, res) => {
  ...
  try {
    ...
    const token = createToken(user.studentID);
    res.cookie('jwt', token, { httpOnly: true, maxAge: maxAge * 1000 });
    res.status(200).json({ user: user.studentID });
  } 
  catch (err) {
    const errors = handleErrors(err);
    res.status(400).json({ errors });
